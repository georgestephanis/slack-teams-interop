...
- **Files are relayed as links, not transferred.** With `syncFiles`, `BridgeCore.withAttachmentLines` appends `📎` lines to the content in the *source* dialect before translation, so they survive edits and footer re-renders. Adapters only populate `NormalizedMessage.attachments`. Real transfer (downloading and re-uploading) is still open in #12.
